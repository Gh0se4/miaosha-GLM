/**
 * Regression tests: src/volc-codingplan-main pricing fetch retry logic.
 *
 * Mirrors the agentplan pricing suite so the codingplan overlay copy of the
 * calculatePriceV5 retry/backoff logic is verified (it was previously
 * untested). fetchPrice must retry transient TypeError network failures with
 * exponential backoff and must not retry aborts or HTTP error responses.
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { loadVolcCodingplanModules, type VolcCodingplanScope } from './_harness';

function flushPromises() {
  return new Promise((resolve) => setTimeout(resolve, 0));
}

function makeAuthSandbox(S: VolcCodingplanScope) {
  (S.document as any).cookie = 'csrfToken=test-csrf; monitor_huoshan_web_id=test-web-id';
}

describe('fetchPrice (codingplan)', () => {
  let S: VolcCodingplanScope;

  beforeEach(() => {
    S = loadVolcCodingplanModules(['10-shared', '20-pricing']);
    makeAuthSandbox(S);
  });
  ...
  it('returns parsed original/current prices on success', async () => {
    ...
    const price = await (S.__volc_codingplan_fetchPrice as (b: unknown) => Promise<unknown>)({
      Product: 'ark_bd',
  ...
  it('retries transient TypeError network failures and eventually succeeds', async () => {
    ...
    const promise = (S.__volc_codingplan_fetchPrice as (b: unknown) => Promise<unknown>)({
      Product: 'ark_bd',
      ConfigurationCode: 'Coding_Plan_Pro_monthly',
    });
  ...
  it('serializes the calculatePriceV5 request body correctly', async () => {
    ...
    await (S.__volc_codingplan_fetchPrice as (b: unknown) => Promise<unknown>)({
      Product: 'ark_bd',
      ConfigurationCode: 'Coding_Plan_Pro_monthly',
...
describe('fetchAllPrices (codingplan)', () => {
  let S: VolcCodingplanScope;
  ...
  it('falls back to zero prices when every fetch fails', async () => {
    ...
    const prices = await (S.__volc_codingplan_fetchAllPrices as (items: unknown[]) => Promise<Record<string, unknown>>)([
