...
> **多平台 Coding Plan 抢购助手** — Chrome MV3 浏览器扩展，辅助抢购 **智谱 bigmodel.cn** 与 **火山引擎 volcengine.com** 的限量套餐（Agent Plan / Coding Plan）。
>
> 扩展在 Chrome 中的显示名为「抢购助手」；仓库沿用历史名「智谱秒杀助手 / miaosha-GLM」。文中「秒杀」指固定时点开售的抢购机制。
...
### 方式一：下载预编译包（最近发布版 v1.4.2）

> 该预编译包为上游仓库最近发布的稳定版（v1.4.2）。要获取当前源码版本（v1.5.0，含本轮全部修复与多平台去重），请用[方式二从源码构建](#方式二从源码构建推荐最新)。
