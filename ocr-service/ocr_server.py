...
def solve():
    ...
    try:
        ...
        if not prompt:
            # 无remark时尝试全图OCR（兼容旧调用）
            img_bytes = base64.b64decode(image_b64)
            boxes = _det.detection(img_bytes)
            if not boxes:
                return jsonify({'success': False, 'error': 'recognition failed'})
            result_str = '|'.join(
                f'{(b[0]+b[2])//2},{(b[1]+b[3])//2}'
                for b in boxes
            )
            elapsed = (time.time() - t0) * 1000
            log.info(f'/solve (no remark): {len(boxes)} boxes, {elapsed:.0f}ms')
            return jsonify({'success': True, 'data': {'result': result_str}})
        ...
        elapsed = (time.time() - t0) * 1000
        log.info(f'/solve remark="{prompt}": {len(points)} points, {elapsed:.0f}ms')

        return jsonify({'success': True, 'data': {'result': result_str}})
    except Exception as e:
        elapsed = (time.time() - t0) * 1000
        log.error(f'/solve error ({elapsed:.0f}ms): {e}')
